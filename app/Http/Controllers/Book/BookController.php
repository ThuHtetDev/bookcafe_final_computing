...
class BookController extends Controller
{
    ...
    public function adminDashboard(){
        ...
        ->where('books.is_reject','0')
        ->where('books.is_cancel','0')
        ->where('books.is_available','1')
        ->select(DB::raw('count(books.user_id) as count'),'users.id','users.name')
        ->groupBy('users.id')
        ->orderBy('count','desc')
        ->take(4)
        ->get();
        ...
        ->where('rent_books.real_return_date','!=',null)
        ->select(DB::raw('count(rent_books.book_id) as count'),'books.id','books.name')
        ->groupBy('books.id')
        ->orderBy('count','desc')
        ->take(4)
        ->get();
        ...
        ->where('rent_books.has_return','1')
        ->where('rent_books.real_return_date','!=',null)
        ->select(DB::raw('count(rent_books.user_id) as count'),'users.id','users.name')
        ->groupBy('users.id')
        ->orderBy('count','desc')
        ->take(4)
        ->get();
