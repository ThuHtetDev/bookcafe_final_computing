<template>
    <div>
          <h3 class="card-title" v-if="notFound">
                 <router-link to="/book-request"><i class="fas fa-arrow-left"></i> </router-link>
                ...
                <div class="card card-primary card-outline" >
                    <div class="row" v-if="detailBooks.front_cover != null && detailBooks.back_cover != null">
                      ...
                      <!-- <div class="card-body box-profile" v-if="images.length > 0">
                        ...
                        <button @click="editForm(detailBooks.id)" v-if="!isPublish" class="btn btn-primary btn-block">Approve</button>
...
<script>
    export default {
        ...
        methods: {
          showFrontImg (index) {
              this.imgs  = [
                  '/storage/user_'+this.registeredUser.id+'/book/front_cover/'+this.detailBooks.front_cover,
              ];
            this.index = index
            this.visible = true
          },
          showBackImg (index) {
              this.imgs  = [
                  '/storage/user_'+this.registeredUser.id+'/book/back_cover/'+this.detailBooks.back_cover,
              ];
            this.index = index
            this.visible = true
          },
          handleHide () {
            ...
            editForm: function(id){
                swal.fire({
                    title: 'Are you sure?',
                    text: 'This Book Request will be Published',
                    type: 'info',
            ...
            publishPost: function(id){
               ...
               axios.post("/_bookcafe/book/publish",{
                    id : id
                  })
                    .then((response) => {
                      if(response.data.success == 'true'){
                        this.$Progress.finish();
                        this.isPublish = true;
                        this.publishAdmin = response.data.publish_admin.name;
                        swal.fire({
                            position: 'middle',
                            icon: 'success',
                            title: this.detailBooks.name +' is successfully published by' + this.publishAdmin,
                            showConfirmButton: false,
                            timer: 2000
                        });
                         this.$router.push('/book-request');
                      }
                    })
                  .catch((error) => {
                          console.log(error);
            ...
            axiosReject: function(bookId){
                 axios.post("/_bookcafe/book/reject",{
                    id : bookId
                  })
                  .then((response) => {
            ...
            rejectForm: function(bookId){
                 swal.fire({
                    title: 'Are you sure?',
                    text: 'This Book Request will be rejected by You',
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonText: 'Yes I will reject it!',
                    cancelButtonText: 'No, Keep it!',
                    showCloseButton: true,
                    showLoaderOnConfirm: true
                    }).then((result) => {
                    if(result.value) {
                       this.axiosReject(bookId);
                    } 
                })
            }
        }
